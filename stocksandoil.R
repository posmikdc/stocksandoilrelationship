...
### Data Cleaning
mdy <- mdy(df$date) #Parsing through mdy structured data
dmy <- dmy(df$date) #Parsing through dmy structured data
mdy[is.na(mdy)] <- dmy[is.na(mdy)] #mdy precedence over dmy (corresponds to left join)
df$date <- mdy #Updated date column (in correct date format)
df$date
...
summary(reg1) #We can see that the results are consistent with Bernanke's 2016 findings.
